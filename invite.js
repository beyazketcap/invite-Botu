...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`<a:assassins_Yonok:827100827943960576> ${member} 𝐀𝐬𝐬𝐚𝐬𝐬𝐢𝐧'𝐬 𝐂𝐫𝐞𝐞𝐝 𝐅𝐚𝐦𝐢𝐥𝐲'e katıldı! \n **Davet Eden:** ${invite.inviter}  (**${total + bonus}** Davet!) <:assassins_f:822359346376736788>`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`${member.user}  𝐀𝐬𝐬𝐚𝐬𝐬𝐢𝐧'𝐬 𝐂𝐫𝐞𝐞𝐝 𝐅𝐚𝐦𝐢𝐥𝐲'den ayrıldı! \n **Davet Eden:** ${user} (${Number(total) + Number(bonus)} Davet!) <:assassins_g:822359310935261195>`)
     }
});
